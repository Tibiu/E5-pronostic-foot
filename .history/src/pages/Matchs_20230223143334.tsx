...
const Tab2: React.FC = () => {
  ...
  const getPredictionLabel = (matchId: number) => {
    const prediction = predictions.find(p => p.matchId === matchId);
    if (prediction) {
      return ` - Pronostic : ${prediction.homeTeamPrediction}-${prediction.awayTeamPrediction}`;
    }
    return '';
  };
  ...
  return (
    <IonContent>
      ...
      <IonHeader>
        ...
        <IonItem>
  <IonLabel position="floating">Pronostic de l'équipe 1</IonLabel>
  <IonInput value={homeTeamPrediction} onIonChange={e => setHomeTeamPrediction(e.detail.value!)} />
</IonItem>

<IonItem>
  <IonLabel position="floating">Pronostic de l'équipe 2</IonLabel>
