...
const Tab2: React.FC = () => {
  ...
  const handlePredictMatch = useCallback((match: Match) => {
    const [homeTeamPrediction, setHomeTeamPrediction] = useState<string>('');
    const [awayTeamPrediction, setAwayTeamPrediction] = useState<string>('');
    const [predictionSubmitted, setPredictionSubmitted] = useState<boolean>(false);
  
    const handleSavePrediction = useCallback(() => {
      // ici, vous pouvez traiter la soumission de la prédiction, par exemple en l'envoyant à un serveur
  ...
  return (
    <IonContent>
      ...
      <IonItem>
        <IonLabel position="floating">{match.homeTeam}</IonLabel>
        <IonInput value={homeTeamPrediction} onIonChange={e => setHomeTeamPrediction(e.detail.value!)} />
      </IonItem>

      <IonItem>
        <IonLabel position="floating">{match.awayTeam}</IonLabel>
        <IonInput value={awayTeamPrediction} onIonChange={e => setAwayTeamPrediction(e.detail.value!)} />
      </IonItem>
      ...
      {predictionSubmitted &&
        <IonText>Votre prédiction a été enregistrée.</IonText>
      }
    </IonModal>
      <IonList>
        {matches.map(match => (
