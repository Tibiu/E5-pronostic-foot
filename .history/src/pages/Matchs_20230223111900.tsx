...
const Tab2: React.FC = () => {
  ...
  const handleSavePrediction = useCallback(() => {
    if (currentPredictionMatch) {
      const updatedMatches = matches.map(match => {
        if (match.id === currentPredictionMatch.id) {
          const updatedPredictions = {
            ...match.predictions,
            [userId]: prediction
          };
          return { ...match, predictions: updatedPredictions };
        } else {
          return match;
        }
      });
    ...
    setPrediction('');
    setCurrentPredictionMatch(undefined);
  }, [currentPredictionMatch, prediction, matches, userId]);
